...
public class Storage {
	...
	private void setConfigByJson(){
		...
		String defaultMode = "";
		try {			
			String configFilePath = Environment.getExternalStorageDirectory().toString() + "/Documents/consent_config.json";	
			...
			if(configFile.exists() && configFile.isFile()){
				...
				String appVersionName = CommonUtil.getInstance(context).getCurrentVersionName("kr.co.less.consent.hch");
				...
				if("DEV".equals(mode)) {
					...
					is = context.getResources().openRawResource(R.raw.dev_configurations);
				}else {
					Log.i(TAG, "[운영버전 설정 정보]");
					is = context.getResources().openRawResource(R.raw.real_configurations);
				}
			}
	...
	public String getStorage(String key){	
		...
		if(value.equals("")){		
			...
			if(key.toLowerCase(Locale.getDefault()).indexOf("macaddress") > -1){
		    	...
		    	value = CommonUtil.getInstance(context).getCurrentVersionName("kr.co.less.consent.hch");
		    	editor.putString(key, value);
		    	editor.commit();
