	...
	$('.deptChoice_enter').click(function () {
		...
		if(applyduty=="Y"){
			$.ajax({
				...
				type: 'post',
				data: 'submit_id=DRZSU11708&business_id=zz&dutplceinstcd=205&dutplcecd=' + DeptCode + '&userid=' + $('#_loginId').val(),
				dataType: 'xml',
				timeout: 10000,
				async: false,
	...
	if (checkVal < -90) {
		alert("WIFI 수신 감도가 좋지 않습니다. WIFI 통신이 잘 되는 곳에서 사용 바랍니다.");
			...
			if ($("#_btnUpdate").is(':visible')) {
				...
				if (id != "" && pw != "") { 
					COMMON.plugin.loadingBar("show", "로그인 중입니다.");
					if (isRun == true) {
						return;
					}
					isRun = true; 
...
function fnGoPage(params) {
	...
	COMMON.plugin.loadingBar("show", "페이지 이동 중입니다.");
